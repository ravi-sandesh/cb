  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && base === '/api/register') {
      try {
        ...
        const out = auth.registerUser(store, username, password);
    ...
    if (req.method === 'POST' && base === '/api/login') {
      try {
        const { username, password } = await json(req);
        const out = auth.loginUser(store, username, password);
    ...
    if (req.method === 'POST' && base === '/api/match/create') {
      ...
      try {
        ...
        const room = relay.openRoom({ matchId: desc.id, code: desc.code, gridSize });
    ...
    if (req.method === 'POST' && base === '/api/match/join') {
      ...
      try {
        ...
        const room = relay.openRoom({ matchId: desc.id, code: desc.code, gridSize: desc.grid_size });
